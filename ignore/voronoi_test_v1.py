...
class Car:
    def __init__(self, x, y, env_size):
        ...
        self.last_x = x
        self.last_y = y
        # Track the item until delivery is done to compute lifecycle
    ...
    def set_target(self, item):
        ...
        print(f"小车（{self.x:.2f}, {self.y:.2f}）开始前往物品。")
    ...
    def draw(self, screen):
        ...
        # If assigned, draw a circle to indicate a task
...
def draw_quarter_circle(screen, center, radius, color):
    # Draw quarter circle for delivery zone
    points = []
...
def run_experiment_1():
                ...
                if distance <= drone.sensor_range:
                    ...
                    for car, poly in car_polygons.items():
                        if (poly and poly.contains(item_point)
                            ...
                            car.set_target(item)
                            print(f"小车（{car.x:.2f}, {car.y:.2f}）被指派去拾取物品 {idx + 1}。")
                            assigned = True
                            break
                    if assigned:
        ...
        for i, car in enumerate(cars):
            ...
            if car.delivering:
                ...
                if distance_to_target < car.speed:
                    ...
                    print(f"小车（{car.x:.2f}, {car.y:.2f}）已送达物品。")
            ...
            if car.target_item and not car.has_item:
                ...
                if distance < car.size + car.target_item.size:
                    ...
                    print(f"小车（{car.x:.2f}, {car.y:.2f}）已拾取物品。")
    ...
    results_1 = {
        '每分钟送达物品数': boxes_per_minute,
        '任务分配效率': std_task,
        '空闲时间占比': idle_ratio,
        '物品送达效率(%)': box_delivery_efficiency,
        '小车平均行驶距离': avg_distance_per_car,
        '平均物品生命周期(分钟)': avg_box_lifecycle,
        '平均忙碌小车数': avg_busy_cars
    }

    time_axis = np.arange(len(boxes_delivered_list)) / FPS

    return (time_axis, boxes_delivered_list, std_task_list, idle_ratio_list,
            box_delivery_eff_list, busy_cars_list, results_1)


def run_experiment_2():
    # Similar structure, but different assignment logic; lifecycle computed the same way
    ENV_SIZE = 800
    pygame.init()
    screen = pygame.display.set_mode((ENV_SIZE, ENV_SIZE))
    ...
    return (time_axis, boxes_delivered_list, std_task_list, idle_ratio_list,
            box_delivery_eff_list, busy_cars_list, results_2)
